...
def _run(*args: str, timeout: int = 45) -> str:
    return browser.run(SESSION_NAME, *args, timeout=timeout)


def _open(url: str, headed: bool = True) -> None:
    browser.open_url(SESSION_NAME, url, headed=headed)


def _current_url() -> str:
    return browser.current_url(SESSION_NAME)


def _snapshot(urls: bool = False) -> list[dict]:
    return browser.snapshot(SESSION_NAME, urls=urls)


def _find_ref(elements: list[dict], role: str, name_contains: str) -> str:
    return browser.find_ref(elements, role, name_contains)
...
def _focus(ref: str, label: str) -> None:
    browser.focus(SESSION_NAME, ref, label)
